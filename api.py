...
class Nairaland(object):
    ...
    def login(self):
    	...
    	self.user.headers.update({"User-Agent":"Mozilla/5.0 (Macintosh; Intel Mac OS X 10_9_2) AppleWebKit/537.36 (KHTML, like Gecko)   	        Chrome/36.0.1944.0 Safari/537.36"})
    	self.user.post("http://www.nairaland.com/do_login", data=self.login_params)
	return self.user

    def post_topic(self, title, body, board):
        self.user.post("http://www.nairaland.com/do_newtopic", data=dict(title=title, body=body, board=board, session=self.user.cookies["session"]))

    def logout(self):
	self.user.post("http://www.nairaland.com/do_logout", data=self.user.cookies["session"])

    #def change_email(self, email):
	"""This has been disabled on nairaland"""
	#self.user.post("http://www.nairaland.com/do_changeemail", data=dict(email=email, session=self.user.cookies["session"]))

    #def change_password(self, oldpassword, password, password2):
	"""This has been disabled on nairaland"""
	self.user.post("http://www.nairaland.com/do_changepass", data=dict(oldpassword=oldpassword, password=password, password2=password2, 	       		#session=self.user.cookies["session"]))
